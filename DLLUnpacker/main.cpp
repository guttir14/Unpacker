...
LONG Handler2(EXCEPTION_POINTERS* ExceptionInfo) {
	...
	if (ExceptionInfo->ExceptionRecord->ExceptionCode == STATUS_ILLEGAL_INSTRUCTION) {
		...
		if (rip < (UINT64)ImageStart || rip > (UINT64)ImageStart + ImageSize) {
			ExceptionInfo->ContextRecord->Rip = rip + 2;
...
static void ScanImports(void* Start, void* End) {
	...
	if (!IO.Buffer) {
		...
		if (!IO.Heap) ExitProcess(STATUS_ACCESS_VIOLATION);
		IO.Size = 0x8000;
	...
	start:
		...
		if ((UINT64)value >= (UINT64)PackedStart && (UINT64)value < (UINT64)PackedStart + PackedSize) break;
		...
		if (IO.Len * 2 > IO.Size - 0x2000) {
			...
			if (!buf) {
				...
				if (!buf) ExitProcess(STATUS_ACCESS_VIOLATION);
...
static __declspec(noreturn) void Dump() {
	...
	for (UINT32 i = 0; i < nt->FileHeader.NumberOfSections; i++) {
		...
		if (strcmp((char*)section->Name, ".rdata") == 0) {
			...
			UINT8* end = start + ALIGN_DOWN_BY(section->Misc.VirtualSize, 8);
			ScanImports(start, end);
		}
	}

	VirtualProtect(ImageStart, ImageSize, PAGE_READWRITE, &oldProtect);
